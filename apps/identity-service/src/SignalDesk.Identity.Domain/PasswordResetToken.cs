...
public sealed class PasswordResetToken
{
        ...
        Guid? tokenId = null)
    {
        ...
        if (expiresAt <= now)
        {
            throw new IdentityDomainException("Password reset token expiry must be in the future.");
        }
    ...
    public void Consume(DateTimeOffset now)
    {
        ...
        {
            throw new IdentityDomainException($"Password reset token cannot be consumed because it is {state.ToString().ToLowerInvariant()}.");
        }
